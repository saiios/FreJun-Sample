...
@interface Event_Invites ()

@end

@implementation Event_Invites
...
//    NSLog(@"json = %@",json);
    ...
    //Time 1 Label
    //day.frame.origin.y+day.frame.size.height+4*multiplier
    ...
    //name Label
    UILabel *name = [[UILabel alloc]initWithFrame:CGRectMake(leftMargin, editedBy.frame.origin.y+editedBy.frame.size.height+11*multiplier, fullWidth, 20*multiplier)];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
                   ^{
                       ...
                       if(data2)
                       {
                           ...
                           dispatch_async(dispatch_get_main_queue(), ^{
                               [self.navigationController popViewControllerAnimated:YES];
                               [[NSNotificationCenter defaultCenter] postNotificationName:@"accepted" object:self];});
                       }
                   });
}
